...
const Uint32 NOSIDESTATECOLOR = 0x33ffffff;
...
{    
    ...
    for (int i = 0; i < STATECOUNT.x; i++)
    {
        for (int j = 0; j < STATECOUNT.y; j++)
        {
            ...
            {
                filledPolygonColor(Renderer,state[i][j].points[0],state[i][j].points[1],6,NOSIDESTATECOLOR +((i+j+2)%5)*0x000055ff*(i+1)*(j+1));
                if (state[i][j].player == -1)
                {
                    armory_place(Renderer,state[i][j].center,0xaaffffff+((i+j+4)%5)*0x00223355,-1,24);
                }
                else
                {
